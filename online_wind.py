...
def fetch_wind_data(lat, lon, start_date, end_date, height):
    ...
    if height == 10:
        ...
        with np.errstate(divide='ignore', invalid='ignore'):
            alpha = np.where(ws10 > 0, np.log(ws100 / np.where(ws10 > 0, ws10, np.nan)) / np.log(100 / 10), 0.143)
...
def show_online_wind_page():
    ...
    with col1:
        lat = st.number_input("纬度", min_value=-90.0, max_value=90.0, value=39.90, step=0.01)
        height = st.selectbox("评估高度 (m)", [10, 30, 50, 70, 80, 100, 120, 150], index=4)
    with col2:
        lon = st.number_input("经度", min_value=-180.0, max_value=180.0, value=116.40, step=0.01)
        rho = st.number_input("空气密度 (kg/m³)", value=1.225, step=0.001, format="%.3f")

    col3, col4, col5 = st.columns(3)
    with col3:
        start_date = st.date_input("开始日期", value=date(2023, 1, 1), max_value=date(2024, 12, 31))
    with col4:
        end_date = st.date_input("结束日期", value=date(2023, 12, 31), max_value=date(2024, 12, 31))
    with col5:
        turbine_name = st.selectbox("机组选型", list(TURBINE_MODELS.keys()), index=0)
        turbine = TURBINE_MODELS[turbine_name]

    if st.button("🚀 获取数据并生成完整评估报告", type="primary", use_container_width=True):
        if start_date >= end_date:
            st.error("❌ 开始日期必须早于结束日期")
            return

        with st.spinner("🌐 正在获取数据..."):
            df = fetch_wind_data(lat, lon, start_date.strftime("%Y-%m-%d"), end_date.strftime("%Y-%m-%d"), height)
        ...
        # ═══════════════════════════════════════════
        # Tab 1: 数据概览（复用 basic_statistics）
        # ═══════════════════════════════════════════
        st.markdown("---")
        st.markdown("## 📊 数据概览")
        ...
        # ═══════════════════════════════════════════
        st.markdown("---")
        render_tab5(df, turbine, turbine_name, key_prefix="online_")  # ← 传入 key_prefix
        ...
        st.download_button(
            "⬇️ 下载完整数据 (CSV)",
            csv,
            f"wind_{lat}_{lon}_{start_date}_{end_date}.csv",
            "text/csv",
